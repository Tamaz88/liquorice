...
public class Campaign {
     ...
     public static void CampaignSelector() throws IOException, InterruptedException {
          Saves.saveSave(Saves.currentFile, "eastern farms", Main.characterName, Globals.nemesisPercentage, Globals.money, Globals.progress, Globals.plantationSize);
          ...
          Main.formatter.printMulti(30, Main.formatter.createBox(1, 1, List.of("Money: $"+Globals.money, "Land funded: "+Globals.plantationSize, "Chance of encountering Nemesis: "+Globals.nemesisPercentage), TextFormatter.BorderStyle.DOUBLE), TextFormatter.PaddingAlignment.CENTER);

          Main.formatter.printMulti(35, Main.formatter.createBox(20, 4, List.of(), TextFormatter.BorderStyle.DOUBLE), TextFormatter.PaddingAlignment.CENTER);
          Main.formatter.printSelectionMultiLine(36, List.of("Market", "Northern farms", "Eastern farms", "Western farms", " [Exit]  "), List.of("Market", "Northern farms", "Eastern farms", "Western farms", "Main menu"), TextFormatter.PaddingAlignment.CENTER);
     }

     public static List<Product> shop() throws IOException, InterruptedException {
          List<GameClasses.Product> cart = new ArrayList<>();
          List<GameClasses.Product> productChoices = new ArrayList<>(List.of(Products.exit));
          List<String> productOptions = new ArrayList<>(List.of("Exit"));
          List<String> productDescs = new ArrayList<>(List.of("Descriptions:"));
          boolean purchasing = true;

          for (GameClasses.Product product : Products.products) {
               product.adjustPrice(Globals.nemesisPercentage-(int)Math.round(Globals.plantationSize*0.2));
     ...
     public static void playCampaign(List<GameClasses.Entity> enemyPool, List<GameClasses.Entity> nemesisEntity, int requiredProgress, int progression, int wage) throws IOException, InterruptedException {
          ...
          while (User.Alive && !fled && !hasDefeatedNemesis) {
               ...
               } else if (GameClasses.randInt(0, 100)<Globals.nemesisPercentage) {
                    List<GameClasses.Entity> tempNemesisEntity = new ArrayList<>();
                    for (GameClasses.Entity entity : nemesisEntity) {
                         tempNemesisEntity.add(new GameClasses.Entity(entity));
                    } 
                    Combat.battle(User, tempNemesisEntity);
                    if (User.Alive) {
                         hasDefeatedNemesis = true;
                    }
               }
               if (User.Alive) {
                    ...
                    Globals.nemesisPercentage += (GameClasses.randInt(0, 100)>Math.round(Globals.plantationSize*0.5)) ? GameClasses.randInt(0, 1) : 0;
          ...
          if (!User.Alive) {
               ...
               if (Globals.plantationSize>0) {
                    Main.formatter.alert(20, List.of("One plot of liquorice plantations you funded got radied", "due to your incompetence.", "", "(No hard feelings tho .3.)"));
                    Globals.plantationSize--;
               }
               SceneController.loadScene("Campaign selector");
          }
          else {
