...
public class Weapons {
    ...
    public static GameClasses.Equipment nemesis = new GameClasses.Equipment(
        ...
        new GameClasses.Attack(
            ...
            (attacker, target) -> {
                try {
                    if (attacker.getEffect("Slik").count>0) {
                        attacker.Stamina = attacker.MaxStamina;
                        attacker.getEffect("Silk").count -= 1;
                        Main.formatter.confirmInput(Combat.getSubMenuRow()+1, TextFormatter.PaddingAlignment.CENTER, attacker.Name+" replenished all stamina and ammunition."+" [ENTER]");
                        ...
                        Main.formatter.confirmInput(Combat.getSubMenuRow()+1, TextFormatter.PaddingAlignment.CENTER, attacker.Name+" is out of silk for repairs."+" [ENTER]");
                    }
                } catch (IOException | InterruptedException e3) {}
            },
            false,
    ...
    public static GameClasses.Equipment ritualDagger = new GameClasses.Equipment(
        ...
        new GameClasses.Attack(
            ...
            (attacker, target) -> {
                try {
                    Main.formatter.confirmInput(Combat.getSubMenuRow(), TextFormatter.PaddingAlignment.CENTER, attacker.applyEffect(Effects.strength, 2, 2, false)+" [ENTER]");
                } catch (IOException | InterruptedException e2) {}
            },
            false,
            new int[]{1, 3},
    ...
    public static GameClasses.Equipment shotgun = new GameClasses.Equipment(
        ...
        new GameClasses.Attack(
            ...
            (attacker, target) -> {
                try {
                    if (attacker.getEffect("Ammo").count>0) {
                        target.damageSelf(Math.min(15, attacker.getEffect("Ammo").count*3));
                        attacker.getEffect("Ammo").count = Math.max(0, attacker.getEffect("Ammo").count-5);
